...
local function sendFileToDiscord(filename, content, webhook)
    queueWebhook(function()
        ...
        if response.StatusCode == 429 then
            notify("Webhook", "Rate limited, retrying once: " .. filename)
            task.wait(5)
...
local function sendToDiscord(embed, images, webhook)
    ...
    -- Queue image batches (each batch as a separate queued request)
    if #images > 0 then
        notify("Webhook", "Queuing " .. #images .. " image(s) for: " .. tostring(embed.title))
        for i = 1, #images, 10 do
            local batchStart = i
            local batchEnd = math.min(i + 9, #images)
            queueWebhook(function()
                local boundary = "----WebKitFormBoundary" .. httpService:GenerateGUID(false)
                local body = ""
                for j = batchStart, batchEnd do
                    ...
                    body = body .. "--" .. boundary .. "\r\n"
                    body = body .. 'Content-Disposition: form-data; name="file' .. (j - batchStart + 1) .. '"; filename="' .. fileName .. '"\r\n'
                ...
                if not imgOk then
                    notify("Webhook Failed", "Image batch " .. batchStart .. " crashed")
                elseif imgResponse.StatusCode == 429 then
                    notify("Webhook", "Rate limited on images, retrying batch " .. batchStart)
                    task.wait(5)
                    pcall(function()
                        request({
                            Url = webhook,
                ...
                elseif imgResponse.StatusCode ~= 200 and imgResponse.StatusCode ~= 204 then
                    notify("Webhook Failed", "Image batch " .. batchStart .. " status: " .. tostring(imgResponse.StatusCode))
                end
            end)
        end
    end
...
-- FIXED: Parallel image downloads (no sequential wait)
local function getLiveryImages(textureIds, downloadLocation)
    ...
    for side, id in textureIds do
        ...
        -- REMOVED task.wait() -> now truly parallel
    end
...
local function takeAssets()
    ...
    local ELSTable = nil
    local mapTable = nil
    local phase4done = false
    local phase5done = false
    local phase4Error = nil
    local phase5Error = nil

    task.spawn(function()
        local ok, result = pcall(getELS)
        if ok then
            ELSTable = result
        else
            phase4Error = result
            notify("Phase 4 Failed", tostring(result))
        end
        phase4done = true
    end)

    task.spawn(function()
        local ok, result = pcall(getMapTemplates)
        if ok then
            mapTable = result
        else
            phase5Error = result
            notify("Phase 5 Failed", tostring(result))
        end
        phase5done = true
    end)
    ...
    -- Wait for parallel phases 4 & 5 to complete
    repeat task.wait() until phase4done and phase5done
    if phase4Error then
        notify("Phase 4 Failed", "ELS data not available: " .. tostring(phase4Error))
    else
        outputTable.ELS = table.clone(ELSTable)
    end
    if phase5Error then
        notify("Phase 5 Failed", "Map templates not available: " .. tostring(phase5Error))
    else
        outputTable.Map = table.clone(mapTable)
    end
    ...
    -- Send files (queued, no unnecessary waits)
    if webhookEnabled("sendELS") and not phase4Error then
        notify("Webhook", "Queuing ELS file...")
        sendFileToDiscord(safeServerName .. "_ELS.json", httpService:JSONEncode(ELSTable), settings.webhookURL)
    end

    if webhookEnabled("sendMapTemplates") and not phase5Error then
        notify("Webhook", "Queuing Map Templates file...")
    ...
    if webhookEnabled("sendFullOutput") then
        notify("Webhook", "Queuing full output file...")
